...
function play(message, song){
    if(song!=undefined){
        ...
        message.channel.send('No more songs, why not add more using b-queue <title>, and l+music play to start them :D.');
...
module.exports.run = function(bot, command, args, message, updateJSON, addFooter){
  ...
  if(args[0]=='play'){
    if(queue[message.guild.id].length<=0){
      message.channel.send('There are no songs queued! Queue some using l+music queue <song name>');
    }else{
      play(message, queue[message.guild.id][0]);
    }
  }
...
module.exports.help = {
    names: ['music'],
    usage: 'l+music <queue <song>/play/skip/list>',
